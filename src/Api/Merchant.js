// src/Api/Merchant.js
/**
 * Combined Merchant API
 * Includes merchant management, categories, products, profile management, and analytics
 */
...
/**
 * ============================================================================
 * MERCHANT CRUD OPERATIONS (for Admin)
 * ============================================================================
 */
 ...
 * @param {string} merchantData.merchant_name - Merchant name
 * @param {string} merchantData.address - Address
 * @param {string} merchantData.phone - Phone number
 * @param {string} merchantData.email - Email
 ...
 * @param {string} merchantId - Merchant ID to update
 * @param {Object} merchantData - Data to update
...
export async function updateMerchant(merchantId, merchantData) {
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      phone: merchantData.phone,
      email: merchantData.email,
      status: merchantData.status !== undefined ? merchantData.status : true,
    })
    .eq("merchant_id", merchantId)
    .select()
    .single();
...
/**
 * Delete a merchant
 * @param {string} merchantId - Merchant ID to delete
 * @returns {Promise<Object>} Deleted merchant info
 */
...
/**
 * ============================================================================
 * GLOBAL PRODUCT & CATEGORY RETRIEVAL (for Admin Dashboard)
 * ============================================================================
 */

/**
 * Get all products across all merchants (for admin view)
 * @returns {Promise<Array>} Array of all products with merchant info
 */
export async function getAllProducts() {
  try {
    const { data, error } = await supabase
      .from("product")
      .select(
        "product_id, product_name, price, category_id, description, image, merchant_id, status, created_at"
      )
      .order("created_at", { ascending: false });
...
/**
 * Get all categories across all merchants (for admin view)
 * @returns {Promise<Array>} Array of all categories with merchant info
 */
export async function getAllCategories() {
  try {
    const { data, error } = await supabase
      .from("category")
      .select("category_id, name, merchant_id, status, created_at")
      .order("created_at", { ascending: false });
